  ...
  if (isEditable) {
    ...
    if (value.indexOf('.') === -1) {
	  ...
	  const lastSymbol = preDecimal.charCodeAt(preDecimal.length - 1);
  	  const containsSymbol = !(lastSymbol >= 48 && lastSymbol <= 57);
      if (!containsSymbol && !isAllZero(postDecimal)) return `${preDecimal}.${postDecimal}`;
      else return preDecimal;
    }
  }
}
...
export const getTextWithEllipses = (txt: string, maxLength = 7) =>
  txt?.length > maxLength ? `${txt.slice(0, maxLength)}...` : txt;

export const getChainIdFromResponse = (networkString: string): string =>
  networkString?.split('_')[1];

// eslint-disable-next-line max-len
export const getFieldLabelFromFieldTitle = (title: string) =>
  applicantDetailsList.find((detail) => detail.id === title)?.title;
