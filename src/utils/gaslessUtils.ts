...
export const signNonce = async (webwallet: Wallet, nonce: string) => {
    ...
    let nonceSigString: string = await webwallet.signMessage(nonce);
    let nonceSig: ethers.Signature = ethers.utils.splitSignature(nonceSigString);
...
export const sendGaslessTransaction = async (biconomy: any, targetContractObject: Contract, targetContractMethod: string,
    ...
    console.log("HERE2")
    console.log(data, targetContractAddress, scwAddress);
...
export const getEventData = async (receipt: any, eventName: string, contractABI: any) => {
    ...
    const isValidEvent = (item: ethers.utils.Fragment) => {
        let fragmentItem = ethers.utils.Fragment.from(item);
    ...
    let humanReadableABI: string | string[] = abiInterface.format(ethers.utils.FormatTypes.full) // convert to human readable ABI
    if (typeof (humanReadableABI) == "string") {
        return false;
    }

    let abiFragments = humanReadableABI.map(item => ethers.utils.Fragment.from(item));

    let eventFragment = abiFragments.filter(isValidEvent);
    ...
    let eventLogs = receipt.logs.filter(isValidEventInReceipt);

    if (eventLogs.length !== 1) {
        throw Error("Invalid Given Event!");
    }

    let eventLog = eventInterface.parseLog(eventLogs[0]);
