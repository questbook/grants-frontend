...
function EditForm({ workspaceData }: EditFormProps) {
	...
	const [newAbout, setNewAbout] = useState(EditorState.createWithContent(convertFromRaw({
		entityMap: {},
		blocks: [{
			text: '',
			key: 'foo',
...
>([null])
	...
	const [aboutData, setAboutData] = useState(useMemo(() => {
		try {
			...
			if(editedFormData?.about) {
				return EditorState.createWithContent(ContentState.createFromText(editedFormData.about))
			}
	...
	const createNewAbout = async () => {
		...
		const oldAboutString = await JSON.stringify(
			convertToRaw(aboutData.getCurrentContent()),
		)
