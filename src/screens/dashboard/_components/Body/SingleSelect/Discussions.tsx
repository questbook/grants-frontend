...
function Discussions() {
	...
	const buildComponents = () => {
		return (
			...
			>
				...
				>
					<Text fontWeight="500">Discussion</Text>
					...
					{comments.length > 0 && <Divider my={4} color="gray.3" height={1} />}
					...
					{proposalTags?.length > 0 && (
						<Flex mt={4} w="100%">
							...
							<Flex ml={4} direction="column">
								<Text variant="metadata" fontWeight="500" color="gray.6">
	...
	const renderComment = (comment: CommentType, index: number) => {
		const member = comment.workspace.members.find(
			(member) =>
				member.actorId.toLowerCase() === comment.sender?.toLowerCase(),
		);
		...
		return (
			<Flex align="start" mt={index === 0 ? 0 : 4}>
				<Image
					borderRadius="3xl"
					boxSize="36px"
					src={
				...
				<Flex ml={3} direction="column">
					<Flex align="center">
						<Text fontWeight="500">{getCommentDisplayName(comment)}</Text>
						<RoleTag
							role={(comment?.role as Roles) ?? "community"}
							isBuilder={
								proposal?.applicantId === comment?.sender?.toLowerCase()
							}
						/>
						{comment?.timestamp && (
							<Text ml={2} variant="body" color="gray.5">
