...
function FundBuilderDrawer() {
	...
	useEffect(() => {
		...
		// setAmounts(milestoneIndices.map((mi, i) => selectedProposalsData[i]?.milestones[mi]?.amount ? parseInt(selectedProposalsData[i]?.milestones[mi]?.amount) : 0))
		//setAmounts(selectedProposalsData.map((p) => p.milestones.findIndex((m) => parseFloat(m?.amountPaid) === 0) !== -1 ? p.milestones.findIndex((m) => parseFloat(m?.amountPaid) === 0) : 0))
