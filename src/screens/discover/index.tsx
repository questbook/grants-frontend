								...
								>
									...
									</Text>
									{
										(sectionGrants && sectionGrants?.length > 0) ? sectionGrants.map((section, index) => {
											...
											const grants = section[sectionName].grants.filter((grant) => (grant.title + sectionName).toLowerCase().includes(filterGrantName.trim().toLowerCase())).map(grant => ({ ...grant, role: 'community' as Roles }))
											return (
												<Box
													my={6}
													key={index}
